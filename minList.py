class minList:
    ...
    def insert(self, node: 'Node' = None, entry: 'Entry' = None, point: 'Point' = None):
        ...
        self.toDo_list, self.mindist_list = (list(s) for s in zip(*sorted(zip(list1, list2), key=lambda pair: pair[1])))
    ...
    #OPENS UP THE FIRST NODE UNTUL WE REACH AN ENTRY. AN ENTRY IS THEN COMPARED WITH THE REST OF THE SKYLINE ENTRIES.
    ...
    def __repr__(self):
        ...
        for num_nodes in range(len(self.toDo_list)):
            ret += "\nMindist: " + str(self.mindist_list[num_nodes]) + "\n" + self.toDo_list[num_nodes].mbr.__repr__() + "\n"
        return ret