...
class RTree:
        ...
        #CREATES RANDOM RECTANGLE ENTIRES
        def create_random(self, size_num: int, r_seed: int = None):
            random.seed(a = r_seed)
            tmp_entry: Entry = None
            tmp_rect: Rectangle = None
            tmp_point: Point = None
            tmp_entry_list: [Entry] = []
            for i in range(size_num):
                tmp_point_lower_left = Point(randint(1, 98), randint(1, 98))
                tmp_point_upper_right = Point(randint(tmp_point_lower_left.x, 99), randint(tmp_point_lower_left.y, 99))
        ...
        def create_random_points(self, size_num: int, r_seed: int = None):
            ...
            for i in range(size_num):
                ...
                tmp_point_upper_right = Point(tmp_point_lower_left.x, tmp_point_lower_left.y)
                tmp_rect = Rectangle(tmp_point_lower_left, tmp_point_upper_right)
                tmp_letter = "E"+str(i)
                tmp_entry = Entry(tmp_letter, tmp_rect)
                self.insert(tmp_entry)
            ...
            time_elapsed_tree = round(end - start, 3)
        ...
        #PLOTS THE TREE IF THE ENTRIES ARE RECTANGLES
        def show(self):
            self.root.show()
        ...
        def bbs_skyline(self):
            ...
            time_elapsed_skyline = round(end - start, 3)
        ...
        def bbs_skyline_range_query(self, rec: 'Rectangle'):
            ...
            if len(self.root.children) != 0:
                for num_children in range(len(self.root.children)):
                    minlist.insert(self.root.children[num_children], rec.lower_left)
            else:
        ...
        class Node():
            ...
            #INSERTS AN ENTRY INTO THE APPROPRIATE NODE. IF SAID NODE EXCEED MAX_ENTRIES, IT SPLITS INTO TWO NODES
            ...
            def insert(self, entry: Entry, level = 0):
                if len(self.children) == 0 and level == 0:
                    ...
                    if len(self.children[0].children) != 0:
                        ...
                        split_node_a, split_node_b = self.children[index].insert(entry, level + 1)
                        self.mbr.expand_to_contain(entry.mbr)
                        if(split_node_a != None):
                        ...
                        index = self.choose_node(entry)
                        split_node_a, split_node_b = self.children[index].insert_entry(entry)
                        if(split_node_a != None):
                            del self.children[index]
                            self.children.extend([split_node_a, split_node_b])
                            self.mbr.expand_to_contain(split_node_a.mbr)
            ...
            def linear_split_entries(self):
                ...
                leaf_split_1 = self.__class__(mbr = Rectangle.create_mbr_for_entry(self.entries[highest_low_index].mbr))
                leaf_split_1.insert_entry(self.entries[highest_low_index])
                leaf_split_2 = self.__class__(mbr = Rectangle.create_mbr_for_entry(self.entries[lowest_high_index].mbr))
                ...
                for num_entries in range(len(self.entries)):
                    if num_entries != highest_low_index and num_entries != lowest_high_index:
                        tmp_rect_1 = Rectangle(copy.copy(leaf_split_1.mbr.lower_left), copy.copy(leaf_split_1.mbr.upper_right))
                        tmp_rect_2 = Rectangle(copy.copy(leaf_split_2.mbr.lower_left), copy.copy(leaf_split_2.mbr.upper_right))
                        expansion_cost_1 = tmp_rect_1.expansion_area_cost(self.entries[num_entries].mbr)
                        expansion_cost_2 = tmp_rect_2.expansion_area_cost(self.entries[num_entries].mbr)
                        if expansion_cost_1 <= expansion_cost_2:
                            leaf_split_1.insert(self.entries[num_entries])
                            leaf_split_1.mbr.expand_to_contain(self.entries[num_entries].mbr)
                        else:
                            leaf_split_2.insert(self.entries[num_entries])
                            leaf_split_2.mbr.expand_to_contain(self.entries[num_entries].mbr)
            ...
            def linear_split_nodes(self):
                ...
                node_split_1 = self.__class__(mbr = Rectangle.create_mbr_for_entry(self.children[highest_low_index].mbr))
                node_split_1.children.extend([self.children[highest_low_index]])
                node_split_2 = self.__class__(mbr = Rectangle.create_mbr_for_entry(self.children[lowest_high_index].mbr))
                ...
                for num_children in range(len(self.children)):
                    if num_children != highest_low_index and num_children != lowest_high_index:
                        tmp_rect_1 = Rectangle(copy.copy(node_split_1.mbr.lower_left), copy.copy(node_split_1.mbr.upper_right))
                        tmp_rect_2 = Rectangle(copy.copy(node_split_2.mbr.lower_left), copy.copy(node_split_2.mbr.upper_right))
                        expansion_cost_1 = tmp_rect_1.expansion_area_cost(self.children[num_children].mbr)
                        expansion_cost_2 = tmp_rect_2.expansion_area_cost(self.children[num_children].mbr)
                        if expansion_cost_1 <= expansion_cost_2:
                            node_split_1.children.extend([self.children[num_children]])
                            node_split_1.mbr.expand_to_contain(self.children[num_children].mbr)
                        else:
                            node_split_2.children.extend([self.children[num_children]])
                            node_split_2.mbr.expand_to_contain(self.children[num_children].mbr)
            ...
            def choose_node(self, entry: Entry, leaf_index = 0, expansion_cost = MAX_ROOT_AREA):
                for num_entries in range(len(self.children)):
                    if self.children[num_entries].mbr.contains(entry.mbr):
                        ...
                        temp_rect = Rectangle(copy.copy(self.children[num_entries].mbr.lower_left), copy.copy(self.children[num_entries].mbr.upper_right))
            ...
            #PLOTS THE TREE FROM THE ROOT NODE IF THE ENTRIES ARE RECTANGLES
            def show(self, level = 0, ax = None, c = None):
                width, height, area = self.mbr.calculate_properties()
                if level == 0:
                    fig = plt.figure()
                    ax = fig.add_subplot(111, aspect = 'equal')
                    plt.xlim([self.mbr.lower_left.x - 10, self.mbr.upper_right.x + 10])
                    plt.ylim([self.mbr.lower_left.y - 10, self.mbr.upper_right.y + 10])
                    patches = []
                    colors = np.random.rand(100)
                    cmap = plt.cm.RdGy
                    c = cmap(colors)

                ax.add_patch(matplotlib.patches.Rectangle((self.mbr.lower_left.x, self.mbr.lower_left.y), width, height, fill = None, edgecolor = c[level+5]))
                if len(self.children) != 0:
                    for child in self.children:
                        child.show(level + 1, ax, c)
                    if level != 0:
                        return
                else:
                    for entry in self.entries:
                        entry_width, entry_height, entry_area = entry.mbr.calculate_properties()
                        ax.add_patch(matplotlib.patches.Rectangle((entry.mbr.lower_left.x, entry.mbr.lower_left.y), entry_width, entry_height, fill = None, edgecolor = 'blue'))
                    if level != 0:
                        return
                plt.show()
                return      
            ...
            def show_points(self, level = 0, ax = None, c = None):
                ...
                ax.add_patch(matplotlib.patches.Rectangle((self.mbr.lower_left.x, self.mbr.lower_left.y), width, height, fill = None, edgecolor = c[level+1]))
                if len(self.children) != 0:
                    for child in self.children:
                    ...
                    for entry in self.entries:
                        circle = plt.Circle((entry.mbr.lower_left.x, entry.mbr.lower_left.y), 0.5, color = 'blue')
                        ax.add_patch(circle)
                    if level != 0:
                        return
                plt.show()
            ...
            def show_skyline(self, skylines: [Entry] = None, level = 0, ax = None, c = None):
                ...
                for skyline in skylines:
                    circle = plt.Circle((skyline.mbr.lower_left.x, skyline.mbr.lower_left.y), 1, color = 'coral')
                    ax.add_patch(circle)
                plt.show()
                return         
                ...
                if len(self.children) != 0:
                    for num_children in range(len(self.children)):
                        if rec.overlaps(self.children[num_children].mbr):
                            self.children[num_children].range_query(rec, range_entries)
                else:
                    for num_entries in range(len(self.entries)):
                        if rec.contains(self.entries[num_entries].mbr):
                            range_entries.extend([self.entries[num_entries]])
                end = timer()
                time_elapsed_range_query = round(end - start, 3)
